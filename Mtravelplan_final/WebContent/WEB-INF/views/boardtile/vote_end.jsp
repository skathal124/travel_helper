...
Object Oflist=request.getAttribute("end_pollist");
if(Oflist!=null){
	plists=(List<TripPoll>)Oflist;
}
%>	
<c:if test="${empty end_pollist}">
<br/><br/><br/>
<font style="font-size: 14px; font-weight: bolder; margin-left: 1%;">&nbsp;
<img src="image/화살표.png" style="width: 16px; height: 16px; vertical-align: top;">&nbsp;투표(마감)</font><br/><br/>
...
<col width="20"/><col width="40"/><col width="110"/><col width="40"/><col width="40"/><col width="25"/><col width="25"/><col width="25"/><col width="40"/>
   ...
   <tr>
     <td><font size="12px" color="#FF5E00
     
     
     
     ">마감된 투표가 없습니다.</font></td>
   </tr>
...
<colgroup>
<col width="20"/><col width="30"/><col width="100px"/><col width="40"/><col width="40"/><col width="25"/><col width="30"/>
</colgroup>
	

	
   <tr>
      <th>No</th><th>아이디</th><th>질문</th><th>종료일</th><th>투표수</th><th>결과</th><th>삭제</th>
   </tr>
           <%
   for(int i=0; i<plists.size();i++)
   {
	 ...
	 <tr bgcolor='#ffffa2' class="overlay">
      ...
      if(isS || DateUtil.isEnd(poll.getEdate())){//이미 투표 기한이 지남
         %>
         <td><font color="blue" size="1"><%=poll.getQuestion() %></font></td>
         <% 
      }else{//투표할 수 있다.
         %>
      ...
      %>
         ...
         <td><%=DateUtil.pollState(poll.getEdate())%><font size="1px"><%=makeDate(poll.getEdate()) %></font></td>
         <td><%=poll.getPolltotal() %></td>
	               <td>
 		<%
         if(isS || DateUtil.isEnd(poll.getEdate()) ){
            %>
               <a href='voteresult.do?tpseq=<%=poll.getTpseq()%>&tseq=<%=poll.getTseq()%>'>결과</a>
            <% 
         }else{//투표안했다.
            //못본다.
            %>
